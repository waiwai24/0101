...
* 设整数a,m(m>0),模的逆元存在当且仅当(a,m)=1。设此时整数s,t满足as+mt=1此时s被m除余数为a模m逆元
...
* 欧拉定理：设m是正整数，(a, m) = 1
  ...
  $\text{则 }a^{\varphi(m)}\equiv1\mathrm{~(nod~}m).\\\text{这里}\varphi(m)\text{是}\{1,...,\mathrm{m}\}\text{中与}m\text{互素的整数个数，也称之为m的欧拉数}$
...
* 同余方程解的个数不超过m
* 判断是否有解：如果(a, m)|b则同余方程有解
* 确定解的个数：同余方程如果有解，则恰有(a, m)个解
* 全部解为：$x_0,\quad x_0+\frac m{(a,m)},\quad x_0+\frac{2m}{(a,m)},\cdots,\quad x_0+\frac{((a,m)-1)m}{(a,m)}$ 
