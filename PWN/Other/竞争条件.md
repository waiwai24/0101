...
./target_process.sh：不断执行 vul 文件，并正好在/tmp/XYZ 符号链接到/etc/passwd 时以 root 权限写入特权用户
...
利用文件系统的竞争条件，passwd_input 文件中写入 `test::0:0:test:/root:/bin/bash`, 密码也可以是一个无密码账户的魔法值：`U6aMy0wojraho`，一次在/etc/passwd 中添加一个 root 用户 test
