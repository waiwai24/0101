...
* chunk_at_offset 将 ptr + offset 位置作为一个 chunk 来对待
...
* unlink 用于从 bins 中取出 chunk
* P 为要被 unlink 的 chunk
...
__builtin_expect(EXP, N)，允许程序员将最有可能执行的分支告诉编译器，让编译器告诉 CPU 提前加载该分支下的指令, 表示的意思是：EXP == N 的概率很大
...
2. 合并的函数就是malloc_consolidate, 调用它的时候就会对空闲块进行合并，那么这个函数的调用条件有什么呢
   1. malloc large bin的时候，当需要很大的chunk的时候，就会调用这个函数先进行一次合并，看看会不会多出来一些可以用的chunk。
