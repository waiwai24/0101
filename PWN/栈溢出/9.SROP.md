...
* 二是主动发送信号（比如调用 kill 函数、alarm 函数或者使用 kill 程序）
* 三是外部事件触发的信号（如 I/O 设备、其他进程）。通过 Shell 运行的进程，通过键盘输入 CTRL + C 或 CTRL + Z 可以向进程发送 SIGINT 或 SIGTSTP 信号。
...
- 依赖系统调用(syscal)强但对 libc.so 的依赖极少，同时还需要能够控制 rax（方法：像 read 函数的返回值就是 rax，可以通过控制输入大小来控制 rax）
